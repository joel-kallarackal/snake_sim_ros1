...
"""
DH Parameters for four snake modules
      ------------------------------
      | theta |  d  |  a   | alpha |
|-----|-------|-----|------|-------|
|  1  |   0   |  0  |  l4  | 90    |
...
def is_orientation_close(R1, R2, threshold_deg=5):
    R_diff = R1 @ R2.T
    ...
    for t2 in theta:
        for t3 in theta:
            for t4 in theta:
                ...
                if is_orientation_close(R_target, final_T[:3, :3]):
                    x.append(position[0])
                    y.append(position[1])
                    z.append(position[2])
...
# arrow_length = 0.2  # change as needed
...
#     ax.quiver(*pos, *vec, color='b', length=arrow_length, normalize=True)
